...
## Process
First, I wanted to make myself familiar with the equation. I looked at the linked Wikipedia article and broke down the equation into four steps
...
![Equations code snippet](screenshots/main-equation.png) ![Equations code snippet](screenshots/conversion-equation.png)

I tested this formula worked with the data I was given and checked it in this converter: https://www.movable-type.co.uk/scripts/latlong.html
...
Next, I needed to 'translate' the forumla into JavaScript to make sure it worked. I used a few sample data points from the list given to test that it worked. Again, I tested this against the converter linked above.
...
- Third; it performs a function FOR EACH of the users in the user data:
    ...
    - This also checks whether the distance calculated is less than or equal to 100km. If it is less than or equal to 100km, it then produces a new object with the User ID, the distance, and the User's name.
...
(If there is a user at the top of the list which contains only an "_id" and "_v" (as oppposed to "_id", "latitude", "user_id", "name", "longitude", "__v") please perform the following:
  Copy the "id" of this user (the user containing only and "id" and "_v"). Then navigate to the delete request and replace the end of the URL with the content of the id, so the URL should look like this: 'localhost:4000/allusers/5f804d1cdcc06639a4750fac'. Go back to the 'GET ALL USERS' request and hit send again, the same list should be returned, simply without this additional user+.
)
...
- Refactoring my code down into seperate functions in different files; when I did this it messed with my reqests so I decided to leave it all in one place!
